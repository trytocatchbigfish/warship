...
//LCD的画笔颜色和背景色
uint32_t POINT_COLOR = 0xFF000000; //画笔颜色，定义为黑色
uint32_t BACK_COLOR  = 0xFFFFFFFF; //背景颜色，定义为白色
...
void LCD_Init(void)
{
    ...
    if (lcddev.id != 0X9341)            //不是9341,看看是不是ST7789
    {
        LCD_WR_REG(0X04);
        lcddev.id = LCD_RD_DATA();      //dummy read
        ...
        if (lcddev.id != 0x7789)        //也不是ST7789,是不是NT35310
        {
            LCD_WR_REG(0XD4);
            lcddev.id = LCD_RD_DATA();  //dummy read
    ...
    else if (lcddev.id == 0x5310) //5310初始化
    {
        LCD_WR_REG(0xED);
        LCD_WR_DATA(0x01);
        LCD_WR_DATA(0xFE);
    ...
    else if (lcddev.id == 0x5510) //5510初始化
    {
    ...
    else if (lcddev.id == 0X1963) //1963初始化
    {
